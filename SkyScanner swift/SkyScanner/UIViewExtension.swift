...
extension UIView{
    func bouceAnimation(_ delay:double_t){
        ...
        self.transform = CGAffineTransform(scaleX: 0.164, y: 0.164)
        UIView.animate(withDuration: 0.13, delay: delay, options: UIViewAnimationOptions.curveEaseOut, animations: {
            self.isHidden = false
            self.transform =  CGAffineTransform(scaleX: 1.164, y: 1.164)
        }) { (flag:Bool) in
            UIView.animate(withDuration: 0.05, delay: 0, options: UIViewAnimationOptions.curveEaseOut, animations: {
